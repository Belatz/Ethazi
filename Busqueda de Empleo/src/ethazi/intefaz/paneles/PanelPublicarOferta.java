...
public class PanelPublicarOferta extends JPanel implements TieneEmergente {
		...
		btn_crear.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					EmergenteCambios.createWindow("¿Seguro que desea añadir el conocimiento?", (TieneEmergente) padre,
							false);
	...
	/**
	 * Gives functionality to the button 'Publicar' so the offer is uploaded to the
	 * data base.
	 */
	@Override
	public void funcionalidad(boolean aceptado) {
		...
		if (aceptado) {
			boolean valido = true;
			try {
				if (txField_titulo == null || txField_titulo.getText().isEmpty()
						|| UtilidadesBD.toOfertaTitulo(txField_titulo.getText()) != null) {
					valido = false;
					txField_titulo.setText("");
				}
			...
			if (valido) {
				try {
					UtilidadesBD.insertarOferta(new Oferta(txField_titulo.getText(), txArea_descripcion.getText(),
							txField_lugar.getText(), Integer.valueOf(txField_sueldoMax.getText()),
							Integer.valueOf(txField_sueldoMin.getText()),
							Float.parseFloat(txField_experiencia.getText()), txArea_aspectosValorar.getText(),
							txArea_aspectosImpres.getText(), true, (Contrato) combo_contrato.getSelectedItem(),
							(Empresa) Aplicacion.getUsuario(), pa_conocimientos.getConocimientosAnadidos()));
					txArea_aspectosImpres.setText("");
					txArea_aspectosValorar.setText("");
					txArea_descripcion.setText("");
					txField_experiencia.setText("");
